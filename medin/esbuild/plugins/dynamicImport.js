...
const dynamicImport = (options = { matcher: /.+\..+/ }) => ({
  name: 'dynamicImport',
  setup: (build) => {
    ...
    build.onLoad({ filter: /.*/, namespace: 'dynamicImport' }, async (args) => {
      const files = glob(args.pluginData.path, {
        cwd: args.pluginData.resolveDir,
      })
        .sort()
        .filter((path) => options.matcher.test(path))

      const controllerNames = files.map((module) =>
        module.replace('./', '').replace('_controller.js', '').replace(/\//g, '--').replace(/_/g, '-')
      )
